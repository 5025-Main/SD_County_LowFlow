...
#
#for k, v in zip(raingauge_dict.keys(),raingauge_dict.values()):
#    if v=='El_Camino':
#        print k, v
...
#weir_dims = pd.read_excel(maindir'2018 Weir Sites.xlsx',parse_cols='B:D').dropna()
#weir_dims['Site ID'] = weir_dims['Site ID'].str.replace('MS4-','')
...
#fds['Flow_cfs_1'] = (fds['1. Flow Measurement, Volume in mL']/28316.8) / fds['1. Flow Measurement, Time in Seconds'].astype('str').apply(lambda x: pd.Timedelta(x).total_seconds())
#fds['Flow_cfs_2'] = (fds['2. Flow Measurement, Volume in mL']/28316.8) / fds['2. Flow Measurement, Time in Seconds'].astype('str').apply(lambda x: pd.Timedelta(x).total_seconds())
#fds['Flow_cfs_3'] = (fds['3. Flow Measurement, Volume in mL']/28316.8) / fds['3. Flow Measurement, Time in Seconds'].astype('str').apply(lambda x: pd.Timedelta(x).total_seconds())
...
######## SITE NAME HERE #######################
SITE_YOU_WANT_TO_PROCESS = 'LAB_TESTT6'
...
for f in files: 
    ...
    if site_name ==  'SWT-023':
        WL = WL.truncate(dt.datetime(2018,5,14,10,10))
...
# MANUAL CLIPS OF BAD DATA/STORMFLOW  
    try:  
        clips_RF = pd.read_excel(hydrograph_fileoutput_dir+'Offsets and Clip times_RF.xlsx',sheetname='Clips',index_col=0,usecols='A:F')   
    except:
        clips_RF = pd.DataFrame()
        pass
    try:  
        clips_LD = pd.read_excel(hydrograph_fileoutput_dir+'Offsets and Clip times_LD.xlsx',sheetname='Clips',index_col=0,usecols='A:F')   
    except:
        clips_LD = pd.DataFrame()
        pass
    try:
        clips_JN = pd.read_excel(hydrograph_fileoutput_dir+'Offsets and Clip times_JN.xlsx',sheetname='Clips',index_col=0,usecols='A:F')   
    except:
        clips_JN = pd.DataFrame()
        pass
    try:
        clips_combined = pd.read_excel(hydrograph_fileoutput_dir+'Offsets and Clip times.xlsx',sheetname='Clips',index_col=0,usecols='A:F')   
    except:
    ...
    for clip in clips_list_for_site.iterrows():
        ...
        if pd.isnull(clip_start)==False and pd.isnull(clip_end) == False:
            ...
            pass
    start, end = dt.datetime(2019,4,23,0,0), dt.datetime(2019,5,8,23,59)
...
### FINALIZED FLOW OUTPUT
    Corr_flow = WL[['offset_flow','Flow_compound_weir','Flow_compound_clipped']].loc[start:]
...
### Pivot TABLES
    ...
    PivotTable = PivotTable.reindex(col_order,axis=1)
