...
def hover_points(line,display_text):
...
HvF = pd.read_csv('C:/Users/jameson.newtson/Desktop/CountyWeirProcessing/HvF-90degweir.csv',index_col='Level (in)')

## Open spreadsheet
#fds = pd.read_excel(calibration_dir+'Weir Calibration Field Form 2019 '+data_processing_date+'.xlsx')

#local copy
fds = pd.read_excel('C:/Users/jameson.newtson/Desktop/CountyWeirProcessing/Weir Calibration Field Form 2019 '+data_processing_date+'.xlsx')
...
weir_dims = pd.read_excel('C:/Users/jameson.newtson/Desktop/2019 Weir Dims.xlsx',sheetname='May 2019',index_col='Site', parse_cols='A:J')

#%% START HERE - SITE NAME

######## SITE NAME HERE #######################
SITE_YOU_WANT_TO_PROCESS = 'SDG-080'

###############################################
start, end = dt.datetime(2019,5,1,0,0), dt.datetime(2019,5,31,23,59)
...
## Two files so two people can have it open in Excel at one time
    ...
    ## Local file 'C:/Users/alex.messina/Desktop/CountyWeirProcessing/Offsets and Clip times '+data_processing_date+'.xlsx'
    offsets = pd.read_excel('C:/Users/jameson.newtson/Desktop/CountyWeirProcessing/Offsets and Clip times '+data_processing_date+'.xlsx',sheetname='Offsets',index_col=0,parse_cols='A:G')
    ...
    field_meas_level = field_meas_level[(field_meas_level['Datetime']>=dt.datetime(2019,5,1)) & (field_meas_level['Datetime']<=dt.datetime(2019,5,31))]
    field_meas_flow =  field_meas_flow[(field_meas_flow['Datetime']>=dt.datetime(2019,5,1)) & (field_meas_flow['Datetime']<=dt.datetime(2019,5,31))]
...
# MANUAL CLIPS OF BAD DATA/STORMFLOW  
    ...
    clips = pd.read_excel('C:/Users/jameson.newtson/Desktop/CountyWeirProcessing/Offsets and Clip times '+data_processing_date+'.xlsx',sheetname='Clips',index_col=0,parse_cols='A:F')  
...
### PLOT QC hydrograph
    ...
    ax1.plot_date(field_meas_level['Datetime'],field_meas_level['Level_above_V_in'],marker='s',c='r',label='Field measurement')
...
#hover_points(points, list(meas_vals['Datetime']))
...
## Get flow data for the times of the flow measurements
...
one_to_one = ax.plot([0,1000],[0,1000],ls='-',marker='None',color='grey',alpha=0.5,label='1:1')
points = ax.plot(final_flow_vals['Flow_gpm'],final_flow_vals['offset_flow'],ls='None',marker='o',markersize=12,label='Flow')
...
hover_points(points, list(final_flow_vals['Datetime']))
