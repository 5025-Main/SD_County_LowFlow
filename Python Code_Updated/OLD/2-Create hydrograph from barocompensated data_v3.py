...
###############################################
start, end = dt.datetime(2019,5,1,0,0), dt.datetime(2019,6,5,23,59)
#end = dt.datetime(2019,5,31,23,59)
cal_end = dt.datetime(2019,5,31,23,59) # when you want to cut off calibration points
...
# MANUAL DATA OFFSETS TO GET SERIES TO LINE 
    offsets = pd.read_excel('C:/Users/alex.messina/Desktop/CountyWeirProcessing/Offsets and Clip times '+data_processing_date+'.xlsx',sheetname='Offsets',index_col=0,parse_cols='A:G')
    ...
    field_meas_level = field_meas_level[(field_meas_level['Datetime']>=dt.datetime(2019,5,1)) & (field_meas_level['Datetime']<=cal_end)]
    field_meas_flow_QC =  field_meas_flow[field_meas_flow['Datetime']>dt.datetime(2019,5,31)]
    field_meas_flow =  field_meas_flow[(field_meas_flow['Datetime']>=dt.datetime(2019,5,1)) & (field_meas_flow['Datetime']<=cal_end)]
...
US = pd.read_excel(localdir+'Alta May 2019 Flow Deliverable.xlsx', sheetname='MS4-'+site_name, index_col=0, header=0,parse_cols='B:D')
