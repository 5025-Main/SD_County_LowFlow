...
######### UPDATE HERE ###################
start_date, end_date = '2019-05-01', '2019-05-31'
#start_date, end_date = '2019-05-31', dt.date.today().strftime('%Y-%m-%d')
## DOWNLOAD Bonsall manually, for some reason it gets wrong data

daterange = pd.date_range('20190530',dt.datetime.now(),freq='D')
...
#web on top code on bottom
#               https://sandiego.onerain.com/sensor.php?time_zone=US%2FPacific&site_id=11&site=dbbd1242-4f9f-43f9-90b9-8b3ad8eeb0c4&device_id=3&device=1fa07b92-6c0a-4168-932c-8433e1cd6d9a&bin=86400&range=standard&legend=true&thresholds=true&refresh=300&show_raw=true&show_quality=true
#               https://sandiego.onerain.com/sensor.php?time_zone=US%2FPacific&site_id=11&site=dbbd1242-4f9f-43f9-90b9-8b3ad8eeb0c4&device_id=3&device=e1f42d5d-1cb7-4732-ac46-acf743c10d3c&data_start=2019-04-31+00%3A00%3A00&data_end=2019-07-24+23%3A59%3A59&bin=900&range=Custom+Range&legend=true&thresholds=true&refresh=off&show_raw=true&show_quality=true
                ...
                if len(soup.findAll('h1')) > 0:
                    if 'Rain Increment' in soup.findAll('h1')[0].text:        
                        ...
                        WebDriverWait(driver,30).until(expected_conditions.visibility_of_element_located((By.XPATH, '//*[@id="dropdown-button-action-download"]')))
                        ...
                        WebDriverWait(driver, 30).until(expected_conditions.visibility_of_element_located((By.XPATH, '//*[@id="dropdown-menu-action-download"]/li[2]/a')))
...
Site_list = ['Bonita', 'Bonsall', 'Cactus_County_Park',  'Couser_Canyon', 'Deer_Springs', 'El_Camino_del_Norte', 'Flinn_Springs_County_Park', 'La_Mesa', 'Lake_Hodges', 'Los_Coches', 'Rainbow_County_Park', 'Rancho_Bernardo', 'Roads_Div_I', 'San_Marcos_CRS']
...
for SITE in Site_list:
    ...
    print(rain_panda)
    writer = pd.ExcelWriter(raindir+filename, engine='xlsxwriter')
